...
from src.common.activity_assets import resolve_activity_asset_path
...
async def download_image(url: str, suffix: str | None = None) -> BuildImage:
    ...
    local_path = resolve_activity_asset_path(url)
    if local_path is not None:
        cache_key = str(local_path)
        if cache_key not in IMG_CACHE:
            IMG_CACHE[cache_key] = BuildImage.open(local_path).convert("RGBA")
...
async def render_text_and_images(
    text: str,
    pic_urls: list,
    width: int,
    font_size: int,
    ...
    for token in raw_tokens:
        if emoji_dict and token in emoji_dict:
            ...
            if curr_x + w > width and curr_line:
                ...
                curr_x = 0
            curr_line.append(
                {"type": "emoji", "url": emoji_dict[token], "w": w, "x": curr_x}
            )
            curr_x += w
        else:
    ...
    if pic_urls:
        ...
        if count == 1:
            ...
            for i, u in enumerate(pic_urls):
                ...
                img_layout.append(
                    (p_img, (i % cols) * (sz + spacing), (i // cols) * (sz + spacing))
                )
    ...
    for line in lines:
        for el in line:
            if el["type"] == "text":
                ...
                e_img = (await download_image(el["url"])).resize(
                    (emoji_size, emoji_size)
                )
                canvas.paste(
                    e_img,
                    (int(el["x"]), int(y + (line_height - emoji_size) // 2)),
                    alpha=True,
                )
        y += line_height
...
async def render_bilibili_card(item: dict[str, Any]) -> BuildImage:
    ...
    if main_info["avatar_url"]:
        av = (
            (await download_image(main_info["avatar_url"], SUFFIX_AVATAR))
            .circle()
            .resize((80, 80))
        )
        header.paste(av, (0, 10), alpha=True)

    Text2Image.from_text(main_info["username"], 32, fill=(251, 114, 153)).draw_on_image(
        header.image, (100, 15)
    )
    time_str = (
    ...
    Text2Image.from_text(time_str, 22, fill=(153, 153, 153)).draw_on_image(
        header.image, (100, 55)
    )

    # 2. 正文
    content_canvas = await render_text_and_images(
        main_info["text"],
        main_info["pic_urls"],
        main_w,
        28,
        main_info["emoji_dict"],
        (34, 34, 34),
        bg_color,
    )
    ...
    if orig_item:
        ...
        origin_inner = await render_text_and_images(
            o_info["text"],
            o_info["pic_urls"],
            o_inner_w,
            26,
        ...
        if origin_inner:
            origin_canvas = BuildImage.new(
                "RGBA", (main_w, origin_inner.height + 40), bg_color
            )
            origin_canvas.draw_rounded_rectangle(
                (0, 0, main_w, origin_canvas.height), radius=12, fill=o_bg
            )
